        ...
        for dual_node_ptr in positive_dual_nodes.iter() {
            ...
            for &row in constrained_rows.iter() {
                ...
                if !unnecessary_edges.is_empty() {
                    ...
                    let mut vertices: BTreeSet<VertexPtr> = hair_view.get_vertices().iter().map(|e| e.upgrade_force()).collect();
                    let mut edges: BTreeSet<EdgePtr> = hair_view.get_base_view_edges().iter().map(|e| e.upgrade_force()).collect();
