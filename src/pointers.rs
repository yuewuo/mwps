...
pub type FastClearTimestamp = Rational;
...
impl<T: Send + Sync + FastClear> std::hash::Hash for FastClearArcRwLock<T> {
    fn hash<H: std::hash::Hasher>(&self, state: &mut H) {
        let address = Arc::as_ptr(&self.ptr);
        address.hash(state);
    }
...
impl<T: Send + Sync + FastClear> std::hash::Hash for FastClearWeakRwLock<T> {
    fn hash<H: std::hash::Hasher>(&self, state: &mut H) {
        let address = Weak::as_ptr(&self.ptr);
        address.hash(state);
    //    std::ptr::hash(self, state);
...
cfg_if::cfg_if! {
    if #[cfg(feature="unsafe_pointer")] {
        ...
        impl<T: Send + Sync + FastClear> std::hash::Hash for FastClearArcUnsafe<T> {
            fn hash<H: std::hash::Hasher>(&self, state: &mut H) {
                let address = Arc::as_ptr(&self.ptr);
                address.hash(state);        
            }
        ...
        impl<T: Send + Sync + FastClear> std::hash::Hash for FastClearWeakUnsafe<T> {
            fn hash<H: std::hash::Hasher>(&self, state: &mut H) {
                let address = Weak::as_ptr(&self.ptr);
                address.hash(state);
            }
