...
impl ResultVerifier for VerifierActualError {
    fn verify(&mut self, primal_dual_solver: &mut Box<dyn PrimalDualSolver>, syndrome_pattern: &SyndromePattern, error_pattern: &Subgraph, visualizer: Option<&mut Visualizer>) {
        ...
        let actual_weight = Rational::from_i64(self.initializer.get_subgraph_total_weight(error_pattern)).unwrap();
        ...
        if self.is_strict {
            let subgraph_weight = Rational::from_i64(self.initializer.get_subgraph_total_weight(&subgraph)).unwrap();
